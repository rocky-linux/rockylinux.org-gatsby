...
export default function Pagination({ totalCount, currentPage }) {
	...
	return (
		<div className="text-center aria-label=Mattermost pagination-navi">
			<LocalizedLink class={previousPage <= 0 ? 'disabled' : null} to={prevPath}> Previous Page </LocalizedLink>
			|
			<LocalizedLink class={nextPage > totalPages ? 'disabled' : null} to={nextPath}> Next Page </LocalizedLink>
		</div>
	);
}
